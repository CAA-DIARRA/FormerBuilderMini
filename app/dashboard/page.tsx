...
import { Plus, Eye, Share2, QrCode, Trash2, BarChart3, UserRound, FileSpreadsheet, GraduationCap, Settings } from "lucide-react";
import { useRouter } from "next/navigation";

type FormItem = { id: string; title: string; createdAt: string; slug: string; isOpen: boolean; responses: number; status?: "Actif" | "Brouillon"; };

function StatusBadge({ open }: { open: boolean }) {
  const cls = open ? "bg-black text-white" : "bg-neutral-200 text-neutral-700";
  return <span className={`px-3 py-1 rounded-full text-xs font-medium ${cls}`}>{open?"Actif":"Brouillon"}</span>;
}
...
export default function DashboardPage() {
  ...
  useEffect(() => { (async () => {
    ...
    const forms: FormItem[] = json.forms.map((f: any) => ({ ...f, status: f.isOpen ? "Actif" : "Brouillon" }));
    setItems(forms);
  })(); }, []);
  ...
  const onView = (id: string) => { const f = items.find(x => x.id === id); if (f) router.push(`/f/${f.slug}`); };
  const onShare = async (id: string) => {
    const f = items.find(x => x.id === id); if (!f) return;
    const url = `${window.location.origin}/f/${f.slug}`;
    if (navigator.share) { try { await navigator.share({ title: f.title, url }); } catch {} }
    ...
    <div className="min-h-screen bg-neutral-50 p-6 md:p-10">
      <div className="max-w-6xl mx-auto space-y-8">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="flex items-center gap-4 border rounded-2xl p-4 bg-white shadow-sm"><div className="p-2 rounded-xl bg-neutral-100"><FileSpreadsheet className="w-5 h-5"/></div><div><div className="text-sm text-neutral-600">Total Formulaires</div><div className="text-2xl font-semibold">{stats.totalForms}</div></div></div>
          <div className="flex items-center gap-4 border rounded-2xl p-4 bg-white shadow-sm"><div className="p-2 rounded-xl bg-neutral-100"><UserRound className="w-5 h-5"/></div><div><div className="text-sm text-neutral-600">Total Réponses</div><div className="text-2xl font-semibold">{stats.totalResponses}</div></div></div>
          <div className="flex items-center gap-4 border rounded-2xl p-4 bg-white shadow-sm"><div className="p-2 rounded-xl bg-neutral-100"><BarChart3 className="w-5 h-5"/></div><div><div className="text-sm text-neutral-600">Formulaires Actifs</div><div className="text-2xl font-semibold">{stats.active}</div></div></div>
        </div>

        <div className="flex items-center justify-between flex-wrap gap-4">
          <h1 className="text-2xl md:text-3xl font-bold">Mes Formulaires</h1>
          <div className="flex flex-wrap items-center gap-2">
            <button onClick={onNew} className="inline-flex items-center gap-2 rounded-2xl px-4 py-2 bg-black text-white hover:bg-neutral-900"><Plus className="w-4 h-4"/>Nouveau Formulaire</button>
            <button className="inline-flex items-center gap-2 border rounded-xl px-3 py-2 hover:bg-neutral-50"><UserRound className="w-4 h-4"/>Assistant IA</button>
            <button className="inline-flex items-center gap-2 border rounded-xl px-3 py-2 hover:bg-neutral-50"><GraduationCap className="w-4 h-4"/>Synthèse Formations</button>
            <button className="inline-flex items-center gap-2 border rounded-xl px-3 py-2 hover:bg-neutral-50"><Settings className="w-4 h-4"/>Administration</button>
          </div>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {items.map(f => (
            <div key={f.id} className="border rounded-2xl p-5 shadow-sm hover:shadow-md transition bg-white flex flex-col gap-4">
              <div className="flex items-start justify-between gap-3">
                <div><h3 className="text-xl font-semibold leading-tight">{f.title}</h3></div>
