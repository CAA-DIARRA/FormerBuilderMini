  ...
  const T = useMemo(() => {
    if (lang === "en") {
      return {
        ...
        headerLine: (d: string) => {
          const base = `Trainer: ${form?.trainerName ?? ""} • Date: ${d} • Location: ${
            form?.location ?? ""
          }`;

          if (form?.durationStart && form?.durationEnd) {
            const ds = new Date(form.durationStart).toLocaleDateString();
            const de = new Date(form.durationEnd).toLocaleDateString();
            return `${base} • Duration: from ${ds} to ${de}`;
          }
          return base;
        },
    ...
    return {
      ...
      headerLine: (d: string) => {
        const base = `Formateur : ${form?.trainerName ?? ""} • Date : ${d} • Lieu : ${
          form?.location ?? ""
        }`;
